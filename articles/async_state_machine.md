...
Еще интересно, что в CLR предусмотрена возможность [пулинга*](https://github.com/dotnet/runtime/blob/main/src/libraries/System.Private.CoreLib/src/System/Runtime/CompilerServices/PoolingAsyncValueTaskMethodBuilderT.cs#L212) `AsyncStateMachineBox` для минимизации аллокаций (метод [StateMachineBox<TStateMachine> RentFromCache()](https://github.com/dotnet/runtime/blob/main/src/libraries/System.Private.CoreLib/src/System/Runtime/CompilerServices/PoolingAsyncValueTaskMethodBuilderT.cs#L299)).
...
Стоит упомянуть, подход из этой статьи похож на то, что еще в 2017 году делал Сергей Тепляков @SergeyT в [своей статье](https://devblogs.microsoft.com/premier-developer/dissecting-the-async-methods-in-c/). Отличие в том, что в моем примере SM сохранена более аккуратно в первозданном виде, еще сразу внутри SM код сопроважден комментариями, разобран вопрос с аллокациями и приведены разные кейсы.

## Упражнения для самопроверки

<spoiler title="Задачки под спойлером">
...
Материал актуален для версии .NET 8.0.1.

Заканчивая, хочу поблагодарить Марка Шевченко @markshevchenko и Евгения Пешкова @epeshk за ревью этой статьи. А еще, в будущих версиях .NET `async/await` [может](https://github.com/dotnet/runtime/issues/94620) сильно преобразиться.

Спасибо, что дочитали до этого момента! С удовольствием отвечу на ваши вопросы в комментариях.
